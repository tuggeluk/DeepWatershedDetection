...
class musicma(imdb):
    def __init__(self,args, image_set, year, devkit_path=None):
        imdb.__init__(self, 'MUSICMA++' + year + '_' + image_set,args)
        self._year = year
    ...
    def _get_default_path(self):
        ...
        """
        return os.path.join(self.args.data_dir, 'MUSICMA++_' + self._year)
    ...
    def _do_python_eval(self, output_dir='output', path=None):
        ...
        for ovthresh in ovthresh_list:
            ...
            for i, cls in enumerate(self._classes):
                if cls in ['tuple_bracket_line', 'other-clef', 'dotted_horizontal_spanner', 'letter_other', 'trill_wobble', 'numeral_0', 'letter_E', 'letter_S', 'curved-line_(tie-or-slur)',
                           'letter_g', 'arpeggio_wobble', 'transposition_text']:  # or cls[:3] == 'let':
                    continue
                # pdb.set_trace()
                filename = self._get_voc_results_file_template().format(cls)
