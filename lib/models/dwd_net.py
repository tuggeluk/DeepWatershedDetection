...
def build_dwd_net(input,model,num_classes,pretrained_dir,substract_mean = False, individual_upsamp = "False", paired_mode=1,  used_heads=None, sparse_heads="False"):
    ...
    if "RefineNet" in model:
        ...
        g, variables, size_diff = build_u_net(input,tf.constant(1, dtype=tf.float32), channels=3, n_class=256,layers=4, features_root=64,individual_upsamp=individual_upsamp, paired_mode=paired_mode, used_heads=used_heads)
