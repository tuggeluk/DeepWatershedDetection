...
def build_dwd_net(input,model,num_classes,pretrained_dir,substract_mean = False, individual_upsamp = "False", paired_mode=1):
    ...
    if individual_upsamp == "True":
        ...
        with tf.variable_scope('deep_watershed'):
            ...
            for pair_nr in range(paired_mode):
                ...
                network_heads["stamp_directions"]["reg"] = [slim.conv2d(g[0][x], 2, [1, 1], activation_fn=None, scope='direction_'+'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[0]))]

                # energy
                network_heads["stamp_energy"] = dict()
                # energy marker - regression
                network_heads["stamp_energy"]["reg"] = [slim.conv2d(g[1][x], 1, [1, 1], activation_fn=None, scope='energy_reg_'+'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[1]))]
                # energy marker - logits
                network_heads["stamp_energy"]["softmax"] = [slim.conv2d(g[1][x], cfg.TRAIN.MAX_ENERGY, [1, 1], activation_fn=None, scope='energy_logits_'+'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[1]))]

                # bounding boxes
                network_heads["stamp_bbox"] = dict()
                # bbox_size - reg
                network_heads["stamp_bbox"]["reg"] = [slim.conv2d(g[2][x], 2, [1, 1], activation_fn=None, scope='bbox_reg_' +'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[2]))]
                # bbox_size - logits
                network_heads["stamp_bbox"]["softmax"] = [slim.conv2d(g[2][x], 2, [1, 1], activation_fn=None, scope='bbox_logits_' +'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[2]))]

                # semseg
                network_heads["stamp_semseg"] = dict()
                network_heads["stamp_semseg"]["softmax"] = [slim.conv2d(g[3][x], num_classes, [1, 1], activation_fn=None, scope='semseg_' +'pair'+str(pair_nr)+'_' + str(x)) for x in range(0, len(g[3]))]
